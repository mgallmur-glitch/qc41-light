...
class TestScanFileDetectsProhibitedTerms(unittest.TestCase):
    ...
    def test_detects_gtm_price(self):
        v = scan_file("fake.md", "Try the audit for $49 today.\n")
        self.assertTrue(any(x["type"] == "prohibited_gtm" for x in v))

    def test_detects_team_audit_path(self):
        v = scan_file("fake.html", 'href="/teams/#team-audit"\n')
        self.assertTrue(any(x["type"] == "prohibited_gtm" for x in v))

    def test_detects_lead_magnet(self):
        v = scan_file("fake.css", "/* soft lead magnet first */\n")
        self.assertTrue(any(x["type"] == "prohibited_term" for x in v))
